...
1. Origin classification at its first serving yard.
2. (Re)classification at an intermediate yard.
...
Together, blocking, train routing, and train scheduling define how each shipment traverses the designed service network as a complete itinerary. This integrated planning process is often referred to as **trip planning**，which is illustrated in the figure below.
...
The minimum time required to execute a connection is referred to as the minimum connection time. It will be enforced when establishing the shipment itineraries during trip planning. If the minimum connection time at a location cannot be met for a pair of inbound and outbound train, the shipment will select the next available (outbound) train (as specified by its BTA) to continue its journey. It is generally observed for passengers making transfers from one service line to another in public transit as well.

![integrated planning process](/images/integrated-planning.png)
